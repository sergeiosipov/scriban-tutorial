...
Locates the **first** substring matching the pattern and returns an
array: index 0 is the whole match; indices 1+ are capture groups. The
array is empty when there's no match.
...
Returns ALL matches as an array of match-arrays (each shaped like
...
Substitutes every match with the replacement string. The replacement
can use `$1`, `$2`, … to refer to capture groups:
...
Splits a string everywhere the pattern matches, returning an array of
fragments (already covered briefly in
...
Escapes a string so it can be safely embedded as a LITERAL inside a
larger regex. Use this when one part of your pattern comes from
user-supplied input:
...
The inverse — strip the escape backslashes from a pattern to recover
its original form. Useful when displaying or logging a regex.
