...
This lesson covers three bands of flow-control constructs:
...
The scope and composition constructs (`capture`, `readonly`, `import`,
`with`, `wrap`, `ret`, `include`) continue in the companion lesson
[09-composition](/scriban-tutorial/lesson/09-composition).
...
Repeat while a condition stays truthy. The `while` object exposes
iteration state — the same idea as `for.*`, but limited to four fields:

| Field | Meaning |
|---|---|
| `while.index` | 0-based iteration counter |
| `while.first` | `true` on the first iteration |
| `while.even` / `while.odd` | parity of `while.index` |
...
That covers every flow-control construct in Scriban. The companion
lesson — [09-composition](/scriban-tutorial/lesson/09-composition) —
picks up from here with the scope and composition constructs: `capture`,
`readonly`, `import`, `with`, `wrap`, `ret`, and `include`.
